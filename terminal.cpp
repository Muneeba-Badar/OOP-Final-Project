...
void Terminal::addTerminal(SAConnection* conn) {
    ...
    cmd.setCommandText("INSERT INTO Terminals (ID, TerminalNumber) VALUES (:1, :2)");
...
void Terminal::deleteTerminal(SAConnection* conn) {
    ...
    // Use parameterized query to avoid SQL injection
    cmd.setCommandText("DELETE FROM Terminals WHERE ID = :1");
    cmd.Param(1).setAsLong() = deleteTerminalId;
...
void Terminal::printTerminalDetails(SAConnection* conn) {
    ...
    // Use parameterized query to avoid SQL injection
    cmd.setCommandText("SELECT ID, TerminalNumber FROM Terminals");
...
void Terminal::menu(SAConnection* conn) {
    ...
    do {
        ...
        switch (choice) {
            case 1:
                addTerminal(SAConnection* conn);
                break;
            case 2:
                deleteTerminal(SAConnection* conn);
                break;
            case 3:
